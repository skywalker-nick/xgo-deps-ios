#!/usr/bin/env bash
#
# Package the iPhoneOS SDKs into a tar file to be used by `build.sh`.
#
...
if [ -d "Contents/Developer/Platforms/iPhoneOS.platform/Developer/SDKs" ]; then
  pushd "Contents/Developer/Platforms/iPhoneOS.platform/Developer/SDKs" &>/dev/null
else
  if [ -d "../Packages" ]; then
    ...
    if [ $? -ne 0 ]; then
      ...
      SDKDIR=$(find . -name SDKs -type d | grep iPhoneOS | head -n1)
...
SDKS=$(ls | grep -E "^iPhoneOS15.*" | grep -v "Patch")
